...
const FEATURE_TERRAIN = new Set(['planet', 'singularity', 'asteroid'])
...
export function validate(world) {
  ...
  if (!hasNear((t) => t.terrain === 'forest')) v.push('no forest within the start radius')
  if (!hasNear((t) => t.terrain === 'hills')) v.push('no hills within the start radius (gold)')
...
export function yieldOf(tiles) {
  const out = { food: 0, gold: 0, progress: 0 }
  for (const t of tiles) {
